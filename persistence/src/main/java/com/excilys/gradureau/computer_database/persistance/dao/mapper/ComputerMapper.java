...
public class ComputerMapper {
    /**
     * @param res ResultSet filled and with the cursor set on the row
     *            containing the useful data.
     * @return a Computer object filled with data
     * @throws SQLException if parsing the resultset is not possible
     */
    public static Computer valueOf(ResultSet res) throws SQLException {
        Company company = null;
        if (res.getLong("company_id") != 0) {
            company = new Company(res.getLong("company_id"), res.getString("company_name"));
        }
        return new Computer(res.getLong("id"), res.getString("name"),
